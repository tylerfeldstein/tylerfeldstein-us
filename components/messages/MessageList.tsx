  ...
  useEffect(() => {
    ...
    if (messageCount > 0) {
      ...
      if (messageCount > prevMessageCountRef.current) {
        ...
        // Only notify if it's not the current user's message
        if (latestMessage && latestMessage.sender !== user?.id) {
          // Get sender name
          const senderName = latestMessage.senderInfo?.name || "Someone";
        ...
        // If user is at bottom, scroll to new message
        if (isAtBottom) {
      ...
      <AnimatePresence>
        {hasNewUnreadMessages && !isAtBottom && (
          <div className={cn(
            ...
            "bottom-36 md:bottom-32",
            "keyboard-visible:bottom-24"
          )}>
            <NewMessageIndicator
